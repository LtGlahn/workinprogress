...
Jeg har [python-kode](https://github.com/LtGlahn/workinprogress/blob/master/skriveapistatistikk.py) som henter oversikt over alle endringssett med "SINUS" i navn på klient eller eier fra NVDB skriveApi, og lagrer dette som CSV. Her er en analyse av disse dataene.

Først hvilke klienter som har "SINUS" i navnet sitt: 
```{r,message=FALSE}
library(lubridate)
library( ggplot2)
...
data.frame(summary( df$klient ))
```

# Alle endringssett fra SINUS.infra

Alle endringssett som NVDB skriveAPi PROD har mottatt fra SINUS, totalt `r nrow(df)` 
...
myplot3 + labs(  x = 'Antall endringssett SVV-ansatte SINUS.infra per mnd i 2019')

```

```{r}
ggplot( dfsvv ) + aes( x = format( dfsvv$dato, "%W")) + geom_bar()  + labs(  x = 'Antall endringssett SVV-ansatte SINUS.infra per uke i 2019')
```
